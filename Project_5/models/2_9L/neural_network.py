...
def create_network(network_input, n_vocab, weight_file=None):
    ...
    model = Sequential()
    model.add(Bidirectional(LSTM(first_layer, dropout=drop), input_shape=(timesteps, data_dim)))
...
def train_model(model, network_input_r, network_output_r, epochs, batch_size, output_tag, sequence_length):
    ...
    # save history
    history = model.fit(network_input_r, network_output_r, epochs=epochs, batch_size=batch_size, callbacks=callbacks_list, validation_split=0.1)
    np_history = np.array(history)
    history_filepath = output_tag + 'history.txt'
    np.savetxt(history_filepath,np_history, delimeter = ',')
